...
(module j-bob  (submod "j-bob-lang.rkt" j-bob-lang)
  (provide (all-defined-out)
           (all-from-out (submod "j-bob-lang.rkt" j-bob-lang)))
  (defun list0 () '())
  ...
  ; rewrite thm for path
  ; like (if (x) (equal a b) nil)
  (defun follow-prems (path claim thm)
    (if (if? thm)
        (if (prem-A? (if.Q thm) path claim)
            (follow-prems path claim (if.A thm))
            (if (prem-E? (if.Q thm) path claim)
                (follow-prems path claim (if.E thm))
                thm))
        thm))
  ...
  (defun equality/path (claim path thm)
    (if (focus-is-at-path? path claim)
        (rewrite-focus-at-path path claim
                               (equality/equation
                                (find-focus-at-path path claim)
                                (follow-prems path claim thm)))
        claim))

  (defun equality/def (claim path app def)
    (if (rator? def)
        (equality/path claim path
                       ; generating meta-equality?
  ...
  (defun J-Bob/step (defs claim steps)
    (if (defs? '() defs)
        (if (expr? defs 'any claim)
            (if (steps? defs steps)
  ...
  ; trying to rewrite proof of the def to 't
  ; according to steps
  (defun rewrite/prove (defs def seed steps)
    (if (defun? def)
        ; rewriting totality claim
        (rewrite/steps defs
                       (totality/claim seed def)
                       steps)
        ; rewriting induction claim
        (if (dethm? def)
            (rewrite/steps defs
  ...
  ; ----------------------------------------
  ; the heart of the whole j-bob
  
  (defun rewrite/define (defs def seed steps)
    ; if a proof was verified, the definition is added to the set of definitions
    (if (equal (rewrite/prove defs def seed steps)
               (quote-c 't))
        (list-extend defs def)
        defs))

  (defun rewrite/define/opt (defs def seed steps)    
    (if (equal (rewrite/prove defs def seed steps)
               (quote-c 't))
        (list-extend defs def)
        (list3 'define 'pending (rewrite/prove defs def seed steps))))

  (defun rewrite/define+1 (defs1 defs2 pfs)
    (if (equal defs1 defs2)
        ; if at some point there was an error in proofs / stop, returning the current state
        defs1
        (if (atom pfs)
            defs2
  ...
  (defun rewrite/define+1/opt (defs1 defs2 pfs)
    (if (equal defs1 defs2)
        ; if at some point there was an error in proofs / stop, returning the current state
        defs1
        (if (atom pfs)
            defs2
            (if (atom (cdr pfs))
                (rewrite/define+1/opt defs2
                                      (rewrite/define/opt defs2
                                                          (elem1 (car pfs))
                                                          (elem2 (car pfs))
                                                          (cdr (cdr (car pfs))))
                                      (cdr pfs))
  ...
  (defun rewrite/define+/opt (defs pfs)
    (if (atom pfs)
        defs
        (if (atom (cdr pfs))
            (rewrite/define+1/opt defs
                                  (rewrite/define/opt defs
                                                      (elem1 (car pfs))
                                                      (elem2 (car pfs))
                                                      (cdr (cdr (car pfs))))
                                  '())
  ...
  ; defs are definitions taken as axioms (they are not checked)
  ; pfs are definitions in the forms (def seed proofs)
  ; where seed is a totality seed for defuns
  ; and seed is an induction seed for dethm
  ...
  (defun J-Bob/define/opt (defs pfs)
    (if (defs? '() defs)
        (if (proofs? defs pfs)
            (rewrite/define+/opt defs pfs)
            defs)
        defs))
